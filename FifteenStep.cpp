/*!
 * @file FifteenStep.cpp
 *
 * @mainpage Adafruit FifteenStep Library
 *
...
// @access public
//
FifteenStep::FifteenStep() { _init(FS_DEFAULT_MEMORY); }

/*!
 * @brief FifteenStep - An alternative constructor that allows the user to set
 ...
 * @param memory the amount of sram to reserve in bytes
 */
FifteenStep::FifteenStep(int memory) { _init(memory); }
...
/*!
 * @brief Sets the tempo and step count to the default values.
 */
void FifteenStep::begin() { begin(FS_DEFAULT_TEMPO, FS_DEFAULT_STEPS); }
 ...
 */
void FifteenStep::begin(int tempo) { begin(tempo, FS_DEFAULT_STEPS); }
...
/*!
 * @brief IMPORTANT: This method should only be called in the
...
/*!
 * @brief Allows user to dynamiclly set the tempo in
...
/*!
 * @brief Allows user to dynamically set the number of 16th note
...
/*!
 * @brief Allows user to dynamically increase the tempo amount
 * until the max tempo has been reached
 */
void FifteenStep::increaseTempo() { setTempo(_tempo + 5); }

/*!
 * @brief Allows user to dynamically decrease the tempo
 * amount until the minimum (0) tempo has
 * been reached.
 */
void FifteenStep::decreaseTempo() { setTempo(_tempo - 5); }

/*!
 * @brief Allows user to dynamically increase the shuffle
...
/*!
 * @brief Allows user to dynamically decrease the shuffle
...
/*!
 * @brief IMPORTANT: Setting a MIDI handler is required for the sequencer
 to operate.
 *
...
/*!
 * @brief Allows user to specifiy a callback that will be called
...
void FifteenStep::setNote(byte channel, byte pitch, byte velocity, byte step) {
  ...
  for (int i = _sequence_size - 1; i >= 0; i--) {
    ...
    if (_sequence[i].pitch == pitch && _sequence[i].step == position &&
    ...
    // use the free slot
    if (_sequence[i].pitch == 0 && _sequence[i].step == 0 &&
        _sequence[i].channel == 0 && !added) {
...
/*!
 * @brief Pauses and unpauses the sequencer at the current position
 */
void FifteenStep::pause() { _running = _running ? false : true; }

/*!
 * @brief Starts sequencer at position 0
 */
...
/*!
 * @brief Stops sequencer at current position
 */
void FifteenStep::stop() { _running = false; }

/*!
 * @brief Turns all notes off and resets sequence
 */
...
/*!
 * @brief Returns a pointer to the current sequence
 * @return Returns a pointer to the current sequence
 */
FifteenStepNote *FifteenStep::getSequence() { return _sequence; }

/*!
 * @brief Returns the closest 16th note to the
 * present time. This is used to see where to
 * save the new note.
 *
 * @return byte - quantized position
 */
byte FifteenStep::getPosition() { return _quantizedPosition(); }
...
void FifteenStep::_triggerNotes() {
  ...
  // loop through the sequence again and trigger note ons at the current
  // position
  for (int i = 0; i < _sequence_size; ++i) {
    ...
    _midi_cb(_sequence[i].channel, _sequence[i].velocity > 0 ? 0x9 : 0x8,
             _sequence[i].pitch, _sequence[i].velocity);
  }
}
